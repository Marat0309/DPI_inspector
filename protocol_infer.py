...
def infer_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == "tcp" and strong_negative >= 5 and not connect_accepted and not ws_exposed and not grpc_exposed and not grpc_strict_exposed:
        ...
        if foreign_open:
            _add(scores, "no_clear_tunnel_evidence", -0.11, against="still answers broadly on foreign SNI")
        if nosni_open:
            _add(scores, "no_clear_tunnel_evidence", -0.09, against="still answers without SNI")
        if foreign_open and nosni_open:
            _add(scores, "no_clear_tunnel_evidence", -0.14, against="combined broad-SNI behavior weakens ordinary-site hypothesis")
    ...
    if foreign_open and nosni_open:
        for h in hypotheses:
            if h["family"] == "ordinary_web_front":
                ...
                break
        for h in hypotheses:
            if h["family"] == "no_clear_tunnel_evidence":
                h["score"] = round(max(0.0, h["score"] * 0.78), 3)
                if h["confidence"] == "high":
                    h["confidence"] = "medium"
                h["against"] = (h.get("against", []) + ["broad SNI acceptance weakens 'no clear tunnel evidence' confidence"])[:5]
                break
        hypotheses.sort(key=lambda x: x["score"], reverse=True)
    ...
    overall = None
    if top:
        if top["family"] in {"ordinary_web_front", "no_clear_tunnel_evidence"} and top["score"] >= 0.60 and not ws_exposed and not grpc_exposed and not grpc_strict_exposed and not connect_accepted and not (foreign_open and nosni_open) and _surface_risk(findings).get("label") != "high":
